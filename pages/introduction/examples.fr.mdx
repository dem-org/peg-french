...
Dans la section précédente, nous avons présenté et donné des exemples simples sur la façon de prompter les LLMs. 

Dans cette section, nous fournirons de nouveaux exemples d'utilisations de prompts et nous introduirons des concepts clés en cours de route. Souvent, la meilleure façon d'apprendre des concepts est de les illustrer par des exemples. Vous trouverez ci-dessous quelques exemples de la manière dont des prompts bien conçus peuvent être utilisés pour effectuer différents types de tâches.
...
## Résumés de texte
L'une des tâches standard des modèles de génération de langage est le résumé de texte. Le résumé de texte peut être utilisé dans beaucoup de domaines différents comme pour résumer des articles et des concepts résumés rapides et faciles à lire. Essayons une tâche simple à l'aide de prompts.
...
Bien que les modèles de langage soient entraînés pour générer du langage naturel et effectuer des tâches connexes, ils sont également capables d'effectuer des classifications et divers traitements des éléments d'un texte.
...
Nous avons donné l'instruction de classer le texte et le modèle a répondu "Neutre", ce qui est correct. Il n'y a rien de mal à cela, mais disons que ce dont nous avons vraiment besoin, c'est que le modèle donne l'étiquette dans le format exact que nous voulons. Ainsi, au lieu de `Neutre`, nous voulons qu'il renvoie `neutre`. Comment y parvenir ? Il y a plusieurs façons de le faire. Nous nous intéressons ici à la spécificité, donc plus nous pouvons fournir d'informations au prompt, meilleurs seront les résultats. Nous pouvons essayer de fournir des exemples pour spécifier le comportement correct. Essayons à nouveau :
...
C'est parfait ! Cette fois-ci, le modèle a retourné `neutre`, ce qui est l'étiquette spécifique que je recherchais. Il semble que l'exemple fourni dans le prompt ait aidé le modèle à être spécifique dans son output. Pour comprendre pourquoi il est parfois important d'être précis, regardez cet exemple et repérez le problème :
...
## Conversation
L'une des choses les plus intéressantes que vous puissiez réaliser avec le prompt engineering est d'indiquer au système LLM comment se comporter, son intention et son identité. Ceci est particulièrement utile lorsque vous construisez des systèmes conversationnels comme les chatbots du service client.

Par exemple, créons un système conversationnel capable de générer des réponses plus techniques et scientifiques aux questions. Remarquez que nous lui indiquons explicitement comment se comporter par le biais de l'instruction. C'est ce que l'on appelle parfois le "role prompting".
...
Je pense que nous avons fait des progrès. Vous pouvez continuer à améliorer vos prompts. Je suis sûr que si vous essayez d'autres exemples, vous obtiendrez de meilleurs résultats. 

---

## Génération de code
Un domaine où les LLMs sont très efficaces est la génération de code. "Copilot" en est un excellent exemple. Il existe un grand nombre de tâches de génération de code que vous pouvez effectuer avec des prompts intelligents. Voyons quelques exemples ci-dessous.
...
C'est très impressionnant. Dans ce cas, nous avons fourni des informations sur le schéma de la base de données et nous lui avons demandé de générer une requête MySQL valide. 
